...
export default function PreGame({ words, levelInfo, onStartFlash, onStartBattle, onBack }) {
  ...
  const selectCard = (i) => {
    setCardIdx(i);
    setFlipped(false);
    speak(words[i].english);
    scrollRef.current?.scrollTo(0, 0);
  };

  const next = () => {
    setFlipped(false);
    if (isLast) setAllSeen(true);
    ...
    <>
      ...
      >
        <div style={{
          ...
          opacity: flipped ? 0.12 : 0.08, transition: 'opacity 0.3s',
        }} />
  ...
  const wordListSection = (
    <>
      <div style={{ fontSize: 12, fontWeight: 700, color: '#bbb', letterSpacing: 1, marginBottom: 10 }}>
        今回の出題単語 ({words.length}語)
      </div>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 960, margin: '0 auto', display: 'flex', gap: 10 }}>
          <button onClick={onStartFlash} style={{
            ...
            <span style={{ fontSize: 18 }}>⚡</span> FlashInput
